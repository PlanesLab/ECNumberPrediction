...
# n_ref replaces the upstream hardcoded reference-DB size (34046, sized for the
# MetaNetX build) so this script works with any precomputed chem_data/ directory.
def add_queries_to_tanimoto(fps, tan_ar, n_ref):
    t0 = time.time()
    metacyc_fps = fps[:n_ref]
    dms_fps = fps[n_ref:]
    ...
    tot_array = np.append(np.array(tan_ar), np.array(tan_ar_dm)[:,:n_ref], axis=0)
...
def main():
    ...
    n_ref = len(tan_ar)
    X_mol = add_queries_to_tanimoto(fp_queries(dms_df, fps, output_dir), tan_ar, n_ref)

    # Update indices for query reactions.
    for i in range(len(dms_df)):
        id_to_index[dms_df.iloc[i, 0]] = i + n_ref
