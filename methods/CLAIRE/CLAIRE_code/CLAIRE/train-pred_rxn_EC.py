...
import time
import sys
sys.path.append('/scratch/jarcagniriv/Case2/CLAIRE')
...
def get_negative_random(anchor_ec, esm_emb):
    result_ec = random.choice(list(esm_emb.keys()))
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # positive sample: numpy expand dims instead of unsqueeze
        pos_arr = random.choice(self.esm_emb[anchor_ec])
        pos = torch.from_numpy(np.expand_dims(pos_arr, axis=0))
        neg_ec = get_negative_random(anchor_ec, self.esm_emb)
        neg_arr = random.choice(self.esm_emb[neg_ec])
        neg = torch.from_numpy(np.expand_dims(neg_arr, axis=0))
...
def main():
    ...
    esm_emb_dict = pickle.load(open('/scratch/jarcagniriv/Case2/CLAIRE/dev/data/pred_metanetx/esm_emb_dict_ec3.pkl', 'rb'))
    model_lookup_train = pickle.load(open('/scratch/jarcagniriv/Case2/CLAIRE/dev/data/pred_metanetx/lookup_array_ec3.pkl', 'rb'))
    labels = pickle.load(open('/scratch/jarcagniriv/Case2/CLAIRE/dev/data/pred_metanetx/labels_train_ec3.pkl', 'rb'))
    print('### load......done')
    ...
    for epoch in range(1, args.epoch + 1):
        if epoch % args.adaptive_rate == 0 and epoch != args.epoch + 1:
            optimizer = torch.optim.Adam(model.parameters(), lr=args.learning_rate, betas=(0.9, 0.999))
            torch.save(model.state_dict(), '/scratch/jarcagniriv/Case2/CLAIRE/dev/results/model/pred_rxn_metanetx/train_{epoch}.pth')
            dataset = Dataset(esm_emb_dict, model_lookup_train, labels)
            train_loader = DataLoader(dataset, batch_size=6000, shuffle=True)
        ...
        if train_loss < best_loss and epoch > 0.8 * args.epoch:
            torch.save(model.state_dict(), '/scratch/jarcagniriv/Case2/CLAIRE/dev/results/model/pred_rxn_metanetx/train_best_{epoch}.pth')
            best_loss = train_loss
            print(f'Best Performance: epoch {epoch:3d} | loss {train_loss:6.8f}')

        elapsed = time.time() - start_time
        print('-' * 65)
        print(f'| epoch:{epoch:2d} | time:{elapsed:3.2f}s | training loss:{train_loss:3.6f}')
        print('-' * 65)
        train_loss_plot.append([epoch, train_loss])

    torch.save(model.state_dict(), '/scratch/jarcagniriv/Case2/CLAIRE/dev/results/model/pred_rxn_metanetx/train_final.pth')
    pickle.dump(train_loss_plot, open('/scratch/jarcagniriv/Case2/CLAIRE/dev/results/model/pred_rxn_metanetx/train_loss_10-4.pkl', 'wb'))
